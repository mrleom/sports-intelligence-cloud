...
| Implementation status | **Partial deterministic coverage.** Current code has a defensive-transition detector, theme-specific setup and coaching language, refined activity names, a specialized final-game name, and a Training Brief pipeline test for compact recovery. |
| Next implementation test needed | Add a guided Session Builder fixture for `Transition to defend: protect central space.` Assert the four activity names, Activity 3 progression, compact Activity 4 structure, and a distinct transition diagram sequence. |
